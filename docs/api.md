...
| PATCH  | `/instances/:id`                                               | settings           | `UpdateInstanceBody` → `{ instance }`; variables the role may not edit are refused (403); refused while running for image/ports/limits/mounts (409); `mounts` only by admins (403) and only under the node's allowed roots (400) |
...
| GET    | `/instances/:id/database`                                      | settings           | `{ database: InstanceDatabaseDto }` (host, port, name, user, password); 404 without a database                                                                                                                                   |
| POST   | `/instances/:id/database/dump`                                 | backups            | `DatabaseDumpBody` (`path` ending in `.sql.gz`, or null for `database-dumps/<name>-<time>.sql.gz`) → `{ path, size }`                                                                                                            |
| POST   | `/instances/:id/database/import`                               | backups            | `DatabaseImportBody` (`path` of a `.sql` or `.sql.gz` in the instance's files) → `{ ok }`; replaces the database's contents                                                                                                      |
